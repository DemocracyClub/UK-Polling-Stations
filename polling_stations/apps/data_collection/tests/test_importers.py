...
from pollingstations.models import PollingDistrict, PollingStation, ResidentialAddress
...
class ImporterTest(TestCase):
    ...
    def test_special_cases(self):
        ...
        districts = PollingDistrict.objects.filter(council_id="X01000000").order_by(
            "internal_council_id"
        )
        self.assertEqual(2, len(districts))
        ...
        stations = PollingStation.objects.filter(council_id="X01000000").order_by(
            "internal_council_id"
        )
