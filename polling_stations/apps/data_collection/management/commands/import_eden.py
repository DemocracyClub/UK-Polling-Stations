...
class Command(BaseMorphApiImporter):
    ...
    elections = ['local.cumbria.2017-05-04']
    ...
    def find_split_districts(self):
        ...
        for station1 in stations:
            station1Codes = [i.strip() for i in station1['Codes'].split("/")[-1].split(",")]
            for station2 in stations:
                station2Codes = [i.strip() for i in station2['Codes'].split("/")[-1].split(",")]
                for station1Code in station1Codes:
                    if station1Code in station2Codes and\
