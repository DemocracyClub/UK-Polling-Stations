...
class Command(BaseCommand):
    def handle(self, *args, **kwargs):
        ...
        update_station_point(
            "E06000010", "8515", Point(-0.321628, 53.787659, srid=4326)
        )
        ...
        update_station_point("E07000012", "6555", Point(0.191974, 52.264660, srid=4326))

        # bug report #5
        print("updating point for: St. Luke's Church Hall, Sefton...")
        update_station_point(
            "E08000014", "4529", Point(-2.985813, 53.646988, srid=4326)
        )

        # 2x reports from Halton council
        print("updating point for: St Marys Halton Ce Primary School, Halton...")
        update_station_point("E06000006", "308", Point(-2.690746, 53.331598, srid=4326))

        print("updating: St Edwards Parish Centre, Halton...")
        stations = PollingStation.objects.filter(
            council_id="E06000006", internal_council_id="283"
        )
        if len(stations) == 1:
            station = stations[0]
        ...
        update_station_point(
            "E09000029", "OB_1", Point(-0.151435, 51.364474, srid=4326)
        )

        # bug report #7
        print("updating: Berkley Street Methodist Church, Huntingdonshire...")
        stations = PollingStation.objects.filter(
            council_id="E07000011", internal_council_id="5860"
        )
        if len(stations) == 1:
            station = stations[0]
        ...
        stations = PollingStation.objects.filter(
            council_id="E06000038", internal_council_id="2701"
        )
        if len(stations) == 1:
            station = stations[0]
            station.address = (
        ...
        update_station_point("E09000022", "VHC", Point(-0.148103, 51.463782, srid=4326))
        ...
        update_station_point(
            "E09000019", "1401", Point(-0.0966823, 51.5559102, srid=4326)
        )

        print("updating: St. Thomas` Church Hall, Islington...")
        stations = PollingStation.objects.filter(
            council_id="E09000019", internal_council_id="1404"
        )
        if len(stations) == 1:
            station = stations[0]
        ...
        update_station_point("E09000020", "672", Point(-0.200082, 51.491363, srid=4326))

        # bug report #13
        print("updating point for: Summertown United Reformed Church, Oxford...")
        update_station_point(
            "E07000178", "4793", Point(-1.265459, 51.778845, srid=4326)
        )

        # bug report #14
        print("updating point for: Cutteslowe Community Centre, Oxford...")
        update_station_point(
            "E07000178", "4796", Point(-1.260158, 51.786878, srid=4326)
        )
        ...
        stations = PollingStation.objects.filter(
            council_id="E08000022", internal_council_id="OE"
        )
        if len(stations) == 1:
            station = stations[0]
            station.postcode = "NE30 4RH"
            station.location = Point(-1.429405, 55.019131, srid=4326)
            station.save()
            print("..updated")
        else:
            print("..NOT updated")
        ...
        update_station_point(
            "E09000010", "5624", Point(-0.069540, 51.652642, srid=4326)
        )

        # feedback
        print("updating point for: All Saints Community Centre, Lewisham...")
        update_station_point(
            "E09000023", "14514", Point(-0.048807, 51.476468, srid=4326)
        )

        # bug report 18
        print("updating: Whiston & Goldsmith Community Hall, Hackney...")
        stations = PollingStation.objects.filter(
            council_id="E09000012", internal_council_id="3055"
        )
        if len(stations) == 1:
            station = stations[0]
        ...
        update_station_point(
            "E09000025", "4000", Point(-0.003917, 51.540183, srid=4326)
        )
