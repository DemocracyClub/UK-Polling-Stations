...
class Command(BaseXpressDemocracyClubCsvImporter):
    ...
    def address_record_to_dict(self, record):
        ...
        if (
            record.addressline6 == "N8 8JP"
            and record.addressline2 == "8 Hornsey Park Road"
        ):
            rec = super().address_record_to_dict(record)
            rec["postcode"] = "N8 0JP"
            return rec
        ...
        if (
            record.addressline6 == "N22 5JH"
            and record.addressline2 == "176 Mount Pleasant Road"
        ):
            rec = super().address_record_to_dict(record)
        ...
        if (
            record.addressline6 == "N17 6PF"
            and record.addressline2 == "36 Downhills Park Road"
        ):
            rec = super().address_record_to_dict(record)
    ...
    def post_import(self):
        ...
        for record in gridrefs:
            ...
            stations = PollingStation.objects.filter(
                council_id=self.council_id, internal_council_id=district_id
            )
            if len(stations) == 1:
                station = stations[0]
                station.location = Point(
                    float(record.east), float(record.north), srid=27700
                )
