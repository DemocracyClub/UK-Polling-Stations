...
class Command(BaseXpressDemocracyClubCsvImporter):
    ...
    def station_record_to_dict(self, record):
        ...
        if record.polling_place_id == "19632":
            record = record._replace(polling_place_easting="382917")
            record = record._replace(polling_place_northing="204928")

        # add point for: France Lynch Church Rooms, Lynch Road, Chalford Hill, Stroud, GL6 8LL
        if record.polling_place_id == "19652":
            record = record._replace(polling_place_easting="390061")
            record = record._replace(polling_place_northing="203128")

        # add point for: Stone Village Hall, Lower Stone Lane, Stone, GL13 9LE
        if record.polling_place_id == "19602":
            record = record._replace(polling_place_easting="368341")
            record = record._replace(polling_place_northing="195188")
