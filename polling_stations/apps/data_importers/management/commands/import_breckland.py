...
class Command(BaseXpressDemocracyClubCsvImporter):
    council_id = "BRE"
    addresses_name = (
        "2026-07-16/2026-06-16T09:59:52.464931/Democracy_Club__16July2026.tsv"
    )
    stations_name = (
        "2026-07-16/2026-06-16T09:59:52.464931/Democracy_Club__16July2026.tsv"
    )
    ...
    def station_record_to_dict(self, record):
        ...
        if record.polling_place_id == "17083":
            record = record._replace(
                polling_place_easting="601036", polling_place_northing="313508"
            )

        if record.polling_place_id in [
            "16814",  # Rocklands Village Hall The Street Rocklands
            "16951",  # Ovington Village Hall Church Road Ovington
            "17079",  # Oxborough Village Hall Swaffham Road Oxborough
        ]:
        ...
        if record.polling_place_id == "17053":
            record = record._replace(
                polling_place_easting="0",
                polling_place_northing="0",
            )
        ...
        if record.polling_place_id == "16848":
            record = record._replace(
                polling_place_easting="0",
                polling_place_northing="0",
            )
