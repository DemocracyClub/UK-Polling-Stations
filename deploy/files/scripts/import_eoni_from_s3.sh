...
cd "/var/www/polling_stations/"

DC_ENVIRONMENT=$DC_ENVIRONMENT
BUCKET='s3://eoni-data.wheredoivote.co.uk.${DC_ENVIRONMENT}'
LATEST_FILE=$(/usr/local/bin/aws s3 ls ${BUCKET} | sort | tail -n1 | rev | cut -d' ' -f1 | rev)
SRCDIR="/var/www/polling_stations/eoni_production_data"
PREM_4326_CSV=${SRCDIR}/PREM_4326.csv
PRO_4326_CSV=${SRCDIR}/PRO_4326.csv
rm -rf ${SRCDIR} && mkdir -p ${SRCDIR}

/usr/local/bin/aws s3 cp ${BUCKET}/"${LATEST_FILE}" ${SRCDIR}
...
manage-py-command import_eoni --cleanup --reprojected eoni_reprojected.csv
