...
psql "$DB" -U "$USER" -c 'DROP INDEX address_location_gist, address_postcode_idx, address_postcode_like_idx, address_uprn_like_idx, uprntocouncil_adv_v_station_idx, uprntocouncil_uprn_like_idx, uprntocouncil_lad_idx;'
...
psql "$DB" -U "$USER" -c 'ALTER TABLE addressbase_uprntocouncil ADD CONSTRAINT uprntocouncil_uprn_fk_address_uprn FOREIGN KEY (uprn) REFERENCES addressbase_address (uprn) MATCH FULL;'

# Rebuild Indexes
psql "$DB" -U "$USER" -c 'CREATE INDEX address_location_gist ON public.addressbase_address USING gist (location);'
psql "$DB" -U "$USER" -c 'CREATE INDEX address_postcode_idx ON public.addressbase_address USING btree (postcode);'
psql "$DB" -U "$USER" -c 'CREATE INDEX address_postcode_like_idx ON public.addressbase_address USING btree (postcode varchar_pattern_ops);'
psql "$DB" -U "$USER" -c 'CREATE INDEX address_uprn_like_idx ON public.addressbase_address USING btree (uprn varchar_pattern_ops);'
psql "$DB" -U "$USER" -c 'CREATE INDEX uprntocouncil_adv_v_station_idx ON public.addressbase_uprntocouncil USING btree (advance_voting_station_id);'
